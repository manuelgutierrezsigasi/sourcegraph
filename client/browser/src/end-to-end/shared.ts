...
export function testSingleFilePage({
    ...
    describe('File views', () => {
        it('adds "View on Sourcegraph" buttons to files', async () => {
            ...
            await getDriver().page.waitForSelector('[data-testid="code-view-toolbar"] .open-on-sourcegraph', {
                timeout: 10000,
            })
            expect(await getDriver().page.$$('[data-testid="code-view-toolbar"] .open-on-sourcegraph')).toHaveLength(1)
            ...
            // await getDriver().page.click('[data-testid="code-view-toolbar"] .open-on-sourcegraph')
                    ...
                    await getDriver().page.evaluate(
                        () =>
                            document.querySelector<HTMLAnchorElement>(
                                '[data-testid="code-view-toolbar"] .open-on-sourcegraph'
                            )?.href
                    ),
